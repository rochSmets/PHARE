...
def setOfModes(polarization, modes, b_amplitudes, theta):
    ...
    wave_num_x = wave_numbers*np.cos(theta)
    wave_num_y = wave_numbers*np.sin(theta)
    ...
    def bx(x, y):
        ...
        for (kx, ky, b) in zip(wave_num_x, wave_num_y, b_amplitudes):
            modes -= b*np.cos(kx*x+ky*y)*np.sin(theta)*polarization
        return modes
