...
public:
    ...
    void computePressure(GridLayout const& layout, double const dt) override
    {
        ...
        auto& TeOld_ = get_from_variants(resources, TeSnapshot_);
        ...
        // snapshot Te before mutating it in place: T_Eq_ below reads neighbor Te
        // values (via derivOnSameCentering) while it writes each cell, so without
        // a read-only copy the sweep would mix this-step/last-step values.
        TeOld_.copyData(Te);

        layout.evalOnBox(Te, [&](auto&... ijk) { T_Eq_(layout, V_, TeOld_, Te, ijk...); });

        // domain-only: Te's ghosts weren't touched by the sweep above, so
        // writing Pe_ over the full array here would overwrite its
        // correctly-filled ghosts with values derived from stale Te ghosts.
        layout.evalOnBox(this->Pe_,
                         [&](auto&... ijk) { this->Pe_(ijk...) = N_(ijk...) * Te(ijk...); });
    }
...
private:
    ...
    // read-only snapshot of Te_, since T_Eq_ mutates Te_ cell by cell while
    // reading its neighbors. Named "PHARE_sumField" to reuse the scratch
    // resource already registered elsewhere (sumField_/tmpField_) instead of
    // allocating a new one -- resource registration is deduplicated by name.
    Field TeSnapshot_{"PHARE_sumField", HybridQuantity::Scalar::P};


    template<typename Field, typename VecField>
    void T_Eq_(GridLayout const& layout, VecField const& Ve, Field const& TeOld, Field& Te,
              auto&... ijk) const
    {
        // dT/dt = -(V.grad)T - (gamma-1)*T*div(V): derived from dP/dt = -(V.grad)P
        // - gamma*P*div(V) combined with mass continuity dN/dt = -(V.grad)N -
        // N*div(V) and T = P/N. advection_/compression_ return the unsigned
        // (V.grad)T / (gamma-1)*T*div(V) terms, so both must be subtracted here.
        Te(ijk...) = TeOld(ijk...)
                    - dt_ * (advection_(layout, Ve, TeOld, ijk...)
                            + compression_(layout, Ve, TeOld, ijk...));
    }
                      ...
                      auto&... ijk) const
    {
        ...
        {
            auto const& Vy = Ve(Component::Y);
            auto gradV_Y   = l_.template derivOnSameCentering<Direction::Y>(Vy, {ijk...});
...
public:
    ...
    // exposed separately so callers needing a level-wide step between
    // sub-steps (e.g. ghost filling) can drive them instead of update() below.
    void computeDensity() { momentModel_.computeDensity(); }
    void computeBulkVelocity(GridLayout const& layout) { momentModel_.computeBulkVelocity(layout); }
    void computePressure(GridLayout const& layout, double const dt)
    {
    ...
    void update(GridLayout const& layout, auto const dt)
    {
        ...
        {
            computeDensity();
            computeBulkVelocity(layout);
            computePressure(layout, dt);
        }
        else
